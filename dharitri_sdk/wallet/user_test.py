...
from dharitri_sdk.core import (Address, Message, MessageComputer, Transaction,
                               TransactionComputer)
...
def test_load_signers_from_pem():
    ...
    assert Address(signers[0].get_pubkey().buffer, "drt").to_bech32() == "drt1qyu5wthldzr8wx5c9ucg8kjagg0jfs53s8nr3zpz3hypefsdd8ssey5egf"
    assert Address(signers[1].get_pubkey().buffer, "drt").to_bech32() == "drt1spyavw0956vq68xj8y4tenjpq2wd5a9p2c6j8gsz7ztyrnpxrruqlqde3c"
    assert Address(signers[2].get_pubkey().buffer, "drt").to_bech32() == "drt1k2s324ww2g0yj38qn2ch2jwctdy8mnfxep94q9arncc6xecg3xaq889n6e"
...
def test_sign_message():
    message = Message("hello".encode(), address=Address.new_from_bech32("drt1qyu5wthldzr8wx5c9ucg8kjagg0jfs53s8nr3zpz3hypefsdd8ssey5egf"))
...
def test_load_secret_key_with_mnemonic():
    ...
    assert UserWallet.load_secret_key(keystore_path, "password", 1).generate_public_key().to_address("drt").to_bech32() == "drt1spyavw0956vq68xj8y4tenjpq2wd5a9p2c6j8gsz7ztyrnpxrruqlqde3c"
    assert UserWallet.load_secret_key(keystore_path, "password", 2).generate_public_key().to_address("drt").to_bech32() == "drt1k2s324ww2g0yj38qn2ch2jwctdy8mnfxep94q9arncc6xecg3xaq889n6e"
    assert UserWallet.load_secret_key(keystore_path, "password", 0).generate_public_key().to_address("drt").to_bech32() == "drt1qyu5wthldzr8wx5c9ucg8kjagg0jfs53s8nr3zpz3hypefsdd8ssey5egf"
