...
def encrypt(data: bytes, password: str, randomness: IRandomness) -> EncryptedData:
    ...
    kdf = Scrypt(salt=randomness.salt, length=kdParams.dklen, n=kdParams.n, r=kdParams.r, p=kdParams.p, backend=backend)
    ...
    cipher = Cipher(algorithms.AES(derived_key_first_half), modes.CTR(randomness.iv), backend=backend)
