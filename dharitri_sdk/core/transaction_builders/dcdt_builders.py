...
from dharitri_sdk.core.interfaces import (IAddress, IGasLimit, IGasPrice,
                                          INonce, ITransactionValue)
from dharitri_sdk.core.serializer import arg_to_string, args_to_strings
from dharitri_sdk.core.transaction_builders.transaction_builder import (
    ITransactionBuilderConfiguration, TransactionBuilder)

logger = logging.getLogger("DCDTIssueBuilder")


class IDCDTIssueConfiguration(ITransactionBuilderConfiguration, Protocol):
    gas_limit_dcdt_issue: IGasLimit
    issue_cost: ITransactionValue
    dcdt_contract_address: IAddress


class DCDTIssueBuilder(TransactionBuilder):
    def __init__(self,
                 config: IDCDTIssueConfiguration,
                 issuer: IAddress,
                 token_name: str,
                 token_ticker: str,
                 initial_supply: int,
        ...
        logger.warning("The 'DCDTIssueBuilder' is deprecated and will soon be removed. Please use 'TokenManagementTransactionsFactory' instead.")
        self.gas_limit_dcdt_issue = config.gas_limit_dcdt_issue

        self.sender = issuer
        self.receiver = config.dcdt_contract_address
    ...
    def _estimate_execution_gas(self) -> IGasLimit:
        return self.gas_limit_dcdt_issue
