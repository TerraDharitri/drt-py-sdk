...
from dharitri_sdk.core.constants import (REWA_NUM_DECIMALS,
                                         REWA_TOKEN_IDENTIFIER)
from dharitri_sdk.core.interfaces import INonce, ITokenIdentifier
...
class TokenPayment:
    ...
    def is_rewa(self):
        return self.token_identifier == REWA_TOKEN_IDENTIFIER

    def is_fungible(self):
        return self.token_nonce == 0

    @classmethod
    def rewa_from_amount(cls, amount: Union[Decimal, str, int]) -> 'TokenPayment':
        amount = Decimal(amount)
        amount_as_integer = cls._amount_to_integer(amount, REWA_NUM_DECIMALS)
        return cls.rewa_from_integer(amount_as_integer)

    @classmethod
    def rewa_from_integer(cls, amount_as_integer: int) -> 'TokenPayment':
        return cls(REWA_TOKEN_IDENTIFIER, 0, amount_as_integer, REWA_NUM_DECIMALS)
    ...
    @classmethod
    def meta_dcdt_from_amount(cls, token_identifier: ITokenIdentifier, nonce: int, amount: Union[Decimal, str, int], num_decimals: int) -> 'TokenPayment':
        amount = Decimal(amount)
        amount_as_integer = cls._amount_to_integer(amount, num_decimals)
        return cls.meta_dcdt_from_integer(token_identifier, nonce, amount_as_integer, num_decimals)

    @classmethod
    def meta_dcdt_from_integer(cls, token_identifier: ITokenIdentifier, nonce: int, amount_as_integer: int, num_decimals: int) -> 'TokenPayment':
