...
from dharitri_py_sdk.abi.small_int_values import U16Value, U32Value
...
def test_set_payload_and_get_payload():
    ...
    value = ArrayValue(
        length=3,
        items=[BigUIntValue(4), BigUIntValue(5), BigUIntValue(6)],
        item_creator=lambda: BigUIntValue(),
    )
    ...
    value = ArrayValue(
        length=3,
        item_creator=lambda: StructValue([Field("a", U32Value()), Field("b", BigUIntValue())]),
    )

    value.set_payload([{"a": 1, "b": 2}, {"a": 3, "b": 4}, {"a": 5, "b": 6}])
    ...
    with pytest.raises(
        ValueError,
        match="populating an array from a native object requires the item creator to be set",
    ):
    ...
    with pytest.raises(
        ValueError,
        match="cannot convert native value to list, because of: 'int' object is not iterable",
    ):
