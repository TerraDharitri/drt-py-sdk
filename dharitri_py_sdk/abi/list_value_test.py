...
def test_set_payload_and_get_payload():
    ...
    value = ListValue(item_creator=lambda: StructValue([Field("a", U32Value()), Field("b", BigUIntValue())]))
    ...
    with pytest.raises(
        ValueError,
        match="populating a list from a native object requires the item creator to be set",
    ):
        value = ListValue()
        value.set_payload([1, 2, 3])
