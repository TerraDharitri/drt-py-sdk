...
from dharitri_py_sdk.abi.fields import (
    Field,
...
def test_set_fields_from_dictionary():
    # Basic
    fields = [Field("a", U32Value()), Field("b", U32Value()), Field("c", U32Value())]
    ...
    with pytest.raises(
        ValueError,
        match=re.escape(
            "cannot set payload for field 'a', because of: invalid literal for int() with base 10: 'foobar'"
        ),
    ):
...
def test_set_fields_from_list():
    ...
    with pytest.raises(
        ValueError,
        match=re.escape("the number of fields (3) does not match the number of provided items (2)"),
    ):
